function [best] = BestPart1ForCase5(test_case,channel,set_energy,x1,y1,x2,y2)
...
if strcmp(test_case,'case5/test_case_05.stk')
    channel=RemoveLine(channel,x1,y1  ,x2,y2);    
else
...
%          set(h,'ShowText','on','TextStep',get(h,'LevelStep')*5);
        ...
        Tmax1(index)=max(max(max(output2)),max(max(output1)));
