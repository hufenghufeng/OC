function [ CE ] = SearchTmaxCE( TmaxConst )
...
%   
    ...
    if (EqualDT(tmaxNow,TmaxConst))
        return;
    else if(tmaxNow>TmaxConst)
            lowerBoundry=CE;
            % search for upper boundry
            while(tmaxNow>TmaxConst)
                lowerBoundry=CE;
                CE=BitControl(CE*1.4);
                [dtNow,tmaxNow]=GetDeltaTandTmax(CE);
                % satisfy constraints?
                if((EqualDT(tmaxNow,TmaxConst))&&(tmaxNow<TmaxConst))
                    return;
                end
            end
            upperBoundry=CE;
    ...
    CE=BinarySearchCETmax(lowerBoundry,upperBoundry,TmaxConst);

end

function [ CE ] = BinarySearchCETmax( lower, upper, TmaxConst )
...
%   
    ...
    while(1)
        ...
        if (EqualCE(tmax,TmaxConst))
            CE=mid;
            return;
        else if (tmax>TmaxConst)
                lower=mid;
            else
                upper=mid;
            end
